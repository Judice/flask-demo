...
class User(UserMixin,db.Model):
    ...
    def is_following(self, user):
        return self.followed.filter_by(followed_id=user.id).first() is not None    #此followed_id是不是Follow中的follow_id??????
    ...
    def followed_posts(self):
        return Post.query.join(Follow, Follow.followed_id == Post.author_id)\
                  .filter(Follow.follower_id == self.id)                        #显示自己关注的人的博客

    def to_json(self):
        json_user = {
            'url': url_for('api.get_users', id=self.id, _external=True),
...
login_manager.anonymous_user = AnonymousUser                         #为什么此处不见括号
...
class Post(db.Model):
    ...
    def from_json(json_post):
        ...
        if body is None or body == '':
            raise ValidationError('Post does not has a body')
