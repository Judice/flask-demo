...
def password_reset_request():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            send_email(user.email,'Reset your password','auth/mail/reset-password',
...
def password_reset(token):
    if not current_user.is_annonymous:
        return redirect(url_for('main.index'))
...
def change_email_request():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.password.data):
            ...
            send_email(new_email,'Confirm your email address','auth/mail/change_email',user=current_user,token=token)
